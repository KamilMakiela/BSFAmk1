...
function [fi_n, s_n, u_n, beta_n, mdd, przep] = GibbsVED1Alt(y, x, n, t, u, beta, par_rozk, fi, s, przep, a1, restr_from, restr, par, mediana)
% disp(beta);
% disp(fi);
% disp(s);
%disp('pass');
...
%         end
    ...
    fi_n(1,1) = randg(1+n*t)/(sum(1*u_n.*(fi(2,1).^(par_rozk)))-mediana);    
    fi_n(2,1) = randg(1+sum(par_rozk))/(par_rozk'*(u_n.*fi_n(1,1)) + 1);
    %disp(fi_n);
    %losuje s^2
    %s0 = 10^(-4) a priori
    %n0 = 10^(-4)
...
%mdd = sum(log(normpdf(y, x*(beta_n)-u_n, ones(size(y))*s_n)));
return